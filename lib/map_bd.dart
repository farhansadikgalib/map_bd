...
class _MapBDState extends State<MapBD> {
  ...
  List<Widget> _buildClickableAreas() {
    ...
    final divisions = [
      {'name': 'Rangpur', 'x': 6.2 + 139.23/2, 'y': 3.12 + 132.73/2, 'width': 139.23, 'height': 132.73},
      {'name': 'Rajshahi', 'x': 1.76 + 138.64/2, 'y': 114.89 + 120.51/2, 'width': 138.64, 'height': 120.51},
      {'name': 'Khulna', 'x': 43.74 + 108.13/2, 'y': 206.29 + 210.79/2, 'width': 108.13, 'height': 210.79},
      {'name': 'Chattogram', 'x': 197.46 + 162.88/2, 'y': 198.71 + 300.01/2, 'width': 162.88, 'height': 300.01},
      {'name': 'Sylhet', 'x': 227.61 + 118.54/2, 'y': 121 + 101.58/2, 'width': 118.54, 'height': 101.58},
      {'name': 'Dhaka', 'x': 100.79 + 151.94/2, 'y': 155.05 + 161.75/2, 'width': 151.94, 'height': 161.75},
      {'name': 'Mymensingh', 'x': 127.1 + 125.93/2, 'y': 102.37 + 96.64/2, 'width': 125.93, 'height': 96.64},
      {'name': 'Barishal', 'x': 144.54 + 86.34/2, 'y': 296.8 + 106.65/2, 'width': 86.34, 'height': 106.65},
    ];
...
class BangladeshMapPainter extends CustomPainter {
  ...
  void _drawSelectedDivision(Canvas canvas, Size size, double pWidth, double pHeight) {
    final divisionMap = {
      'Rangpur': {'painter': RangpurPainter(), 'left': 6.2, 'top': 3.12, 'width': 139.23, 'height': 132.73, 'color': Colors.amber[600]!},
      'Rajshahi': {'painter': RajshahiPainter(), 'left': 1.76, 'top': 114.89, 'width': 138.64, 'height': 120.51, 'color': Colors.orange[600]!},
      'Khulna': {'painter': KhulnaPainter(), 'left': 43.74, 'top': 206.29, 'width': 108.13, 'height': 210.79, 'color': Colors.purple[600]!},
      'Chattogram': {'painter': ChittagongPainter(), 'left': 197.46, 'top': 198.71, 'width': 162.88, 'height': 300.01, 'color': Colors.blue[600]!},
      'Sylhet': {'painter': SylhetPainter(), 'left': 227.61, 'top': 121, 'width': 118.54, 'height': 101.58, 'color': Colors.indigo[600]!},
      'Dhaka': {'painter': DhakaPainter(), 'left': 100.79, 'top': 155.05, 'width': 151.94, 'height': 161.75, 'color': Colors.green[600]!},
      'Mymensingh': {'painter': MymensinghPainter(), 'left': 127.1, 'top': 102.37, 'width': 125.93, 'height': 96.64, 'color': Colors.lime[600]!},
      'Barishal': {'painter': BarisalPainter(), 'left': 144.54, 'top': 296.8, 'width': 86.34, 'height': 106.65, 'color': Colors.teal[600]!},
    };
  ...
  void _drawAllDivisions(Canvas canvas, Size size, double pWidth, double pHeight) {
    ...
    _drawDivision(canvas, size, pWidth, pHeight, 
        RangpurPainter(), 6.2, 3.12, 139.23, 132.73, Colors.amber[600]!);
    _drawDivision(canvas, size, pWidth, pHeight, 
        RajshahiPainter(), 1.76, 114.89, 138.64, 120.51, Colors.orange[600]!);
    _drawDivision(canvas, size, pWidth, pHeight, 
        KhulnaPainter(), 43.74, 206.29, 108.13, 210.79, Colors.purple[600]!);
    _drawDivision(canvas, size, pWidth, pHeight, 
        ChittagongPainter(), 197.46, 198.71, 162.88, 300.01, Colors.blue[600]!);
    _drawDivision(canvas, size, pWidth, pHeight, 
        SylhetPainter(), 227.61, 121, 118.54, 101.58, Colors.indigo[600]!);
    _drawDivision(canvas, size, pWidth, pHeight, 
        DhakaPainter(), 100.79, 155.05, 151.94, 161.75, Colors.green[600]!);
    _drawDivision(canvas, size, pWidth, pHeight, 
        MymensinghPainter(), 127.1, 102.37, 125.93, 96.64, Colors.lime[600]!);
    _drawDivision(canvas, size, pWidth, pHeight, 
        BarisalPainter(), 144.54, 296.8, 86.34, 106.65, Colors.teal[600]!);
  }

  void _drawNationalBorder(Canvas canvas, Size size, double pWidth, double pHeight) {
  ...
  void _drawDivisionalBorders(Canvas canvas, Size size, double pWidth, double pHeight) {
    final borderPaint = Paint()
      ..color = Colors.white
      ..style = PaintingStyle.stroke
      ..strokeWidth = 1.5;
  ...
  void _drawDivision(Canvas canvas, Size size, double pWidth, double pHeight, 
      CustomPainter painter, double left, double top, double width, double height, Color color) {
    ...
    canvas.translate(
      left / pWidth * size.width,
      top / pHeight * size.height,
    );
  ...
  void _drawDivisionNames(Canvas canvas, Size size, double pWidth, double pHeight) {
    final divisions = [
      ...
      {'name': 'Chattogram', 'x': 197.46 + 162.88/2, 'y': 198.71 + 300.01/2},
  ...
  void _drawDivisionData(Canvas canvas, Size size, double pWidth, double pHeight) {
    ...
    for (var division in divisions) {
      ...
      if ((data != null && data.isNotEmpty) && (selectedDivision == null || selectedDivision == divisionName)) {
        Offset center;
